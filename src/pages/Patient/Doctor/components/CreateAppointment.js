                ...
                <div class="modal-content" style={{ border: "2px solid lightgray", margin: '10%', textAlign: 'center' }}>
                    <div class="modal-body">
                        ...
                        {selectedSlot.status === "BOOKED" ? (
                            ...
                            <p>{`Please confirm your appointment with `} <span style={{ fontSize: '1rem', fontWeight: 'bold' }}> {(doctor?.firstName ?? doctor?.name) + " " + (doctor?.lastName ?? "")} </span> on <span style={{ fontSize: '1rem', fontWeight: 'bold' }}>{selectedSlot?.title}</span></p>
                        )}
                        <hr />
